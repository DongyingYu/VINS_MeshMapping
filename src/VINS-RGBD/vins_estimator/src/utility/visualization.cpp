...
ros::Publisher pub_odometry, pub_latest_odometry,pub_odometry2voxgraph;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    // 在pose_graph中发布里程计数据，这里就不发布了
    pub_odometry2voxgraph = n.advertise<nav_msgs::Odometry>("odometry_input_topic", 1000);
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        path.header = header;
        //为便于显示，frame_id更改为与后端一致mission
        path.header.frame_id = "mission";
...
//----------------------------------------------------------------------------------------
void pubOdometry2voxgraph(const Estimator &estimator, const std_msgs::Header &header)
{
    if (pubOdometry_current_time < 0){
        pubOdometry_current_time = header.stamp.toSec();
    }
    ...
    // 多加一个判断条件，只发布关键帧所对应的位姿，即：estimator.marginalization_flag == 0
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)
    {
        ...
        Quaterniond tmp_Q;
        tmp_Q = Quaterniond(estimator.Rs[WINDOW_SIZE - 2]);
        odometry.pose.pose.position.x = estimator.Ps[WINDOW_SIZE - 2].x();
        odometry.pose.pose.position.y = estimator.Ps[WINDOW_SIZE - 2].y();
        odometry.pose.pose.position.z = estimator.Ps[WINDOW_SIZE - 2].z();
        odometry.pose.pose.orientation.x = tmp_Q.x();
        odometry.pose.pose.orientation.y = tmp_Q.y();
        odometry.pose.pose.orientation.z = tmp_Q.z();
        odometry.pose.pose.orientation.w = tmp_Q.w();
        ...
        // odometry.twist.twist.linear.z = estimator.Vs[WINDOW_SIZE].z();
        //"odometry"
        pub_odometry2voxgraph.publish(odometry);
...
void pubTF(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    //广播世界坐标系到IMU坐标系之间的转换关系
    //参数中首先是旋转变换和位移变换信息，旋转使用四元数表示；再者是时间戳信息；母节点存储的参考系；子节点存储的参考系。
