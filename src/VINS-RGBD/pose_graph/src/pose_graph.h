...
public:
	...
	//直接发布pointcloud2点云数据,没有返回值,void类型
	//void PointsToPointCloud(const cv::Mat& color, const cv::Mat& depth, const double& cx, 
	//								   const double& cy, const double& fx, const double& fy, const int ROW, const int& COL );
	
	void PointsToPointCloud(const pcl::PointCloud<PointT>::Ptr &Point, const std_msgs::Header &header);

	void pubKeyframePose(const nav_msgs::Odometry::ConstPtr &pose_msg, const std_msgs::Header &header);
