...
KeyFrame::KeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i, Matrix3d &_vio_R_w_i, cv::Mat &_image,cv::Mat &_image_color,cv::Mat &_depth,
		           ...
		           vector<cv::Point2f> &_point_2d_norm, vector<double> &_point_id, int _sequence)
{
	...
	image = _image.clone();
	image_color = _image_color.clone();
	depth = _depth.clone();
	cv::resize(image, thumbnail, cv::Size(80, 60));
	point_3d = _point_3d;
	point_2d_uv = _point_2d_uv;
	point_2d_norm = _point_2d_norm;
	// 查看point_3d_depth用在哪? 具体起到什么作用?
	...
	computeBRIEFPoint();
	// 这里image好像还不能直接release掉，不光是用于后续的位姿保存时使用，在DEBUG_IMAGE不为0时，在其他地方也会用到；
	// image.release();
	if(!DEBUG_IMAGE){
		image.release();
		image_color.release();
