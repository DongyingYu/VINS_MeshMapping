...
public:
	...
	KeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i, Matrix3d &_vio_R_w_i, cv::Mat &_image, cv::Mat &_image_color, cv::Mat &_depth, vector<cv::Point3f> &_point_3d_depth,
			 vector<cv::Point3f> &_point_3d, vector<cv::Point2f> &_point_2d_uv, vector<cv::Point2f> &_point_2d_normal, 
			 vector<double> &_point_id, int _sequence);
	// 在加载位姿图时会用到该构造函数，所以可以不用对该函数进行改造，有无深度图信息并不影响
