...
std::mutex m_buf;
std::mutex m_process;
std::mutex m_camera_use;
std::mutex m_pointcloud2;
int frame_index  = 0;
int sequence = 1;
PoseGraph posegraph;
...
void process()
{
    ...
    while (true)
    {
        ...
        if (pose_msg != NULL)
        {
            ...
            // 这里的rgb/depth图像都是经过格式转换的，rgb变为了灰度图，深度图变为了opencv中对应的MONO16格式；
            ...
            try
            {
                ...
                cv_ptrRGB = cv_bridge::toCvShare(image_msg,sensor_msgs::image_encodings::BGR8);
            }
            ...
            // try
            // {
            //     cv_ptrD = cv_bridge::toCvShare(depth_msg,sensor_msgs::image_encodings::MONO16);
            // }
            // catch (cv_bridge::Exception& e)
            // {
            //     ROS_ERROR("cv_bridge exception: %s", e.what());
            ...
            // m_camera_use.lock();
            // posegraph.PointsToPointCloud(cv_ptrRGB->image,depth,cx,cy,fx,fy,ROW,COL);
            // m_camera_use.unlock();
            // build keyframe
            // 这里R、T为当前帧的旋转、平移矩阵，在选为关键帧之后可以直接对该数据进行使用，用以表示关键帧所对应的位姿；
            ...
            // 以平移量为判别依据，大于某个阈值，设定为关键帧，进队关键帧组合点云并发布出去
            // 这里的大小在对应的live或data launch文件中修改，修改之后运行会更快一些，冗余信息减少很多
            if((T - last_t).norm() > SKIP_DIS)
            {
                ...
                // 在这里对拿到的深度图及rgb图像进行处理，组合为点云格式；
                for (int j = 0; j < 480; j += 3)
                {
                    //for (int i = L_BOUNDARY; i < COL - R_BOUNDARY; i += PCL_DIST)
                    for (int i = 0; i < 640; i += 3)
                    {
                        //注意:a(i,j)必须是列(COL)值在前,出来的点云才是正确的,即y坐标参数在前,x参数在后
                        Eigen::Vector2d a(i,j);
                        Eigen::Vector3d b;
						//depth is alignedm_camera_use
                        m_camera_use.lock();
                        m_camera->liftProjective(a, b);
                        // 很奇葩,系统代码是以列为基准进行图像的遍历,用指针的方式进行操作有问题
                        // 注意：这里的深度因子设置为1000，是需要根据相机的参数而调整的；
                        float depth_val = ((float)depth.ptr<unsigned short>(j)[i]) / 1000.0;
                        //float depth_val = ((float)depth.at<unsigned short>(j, i)) / 1000.0;
                        // 对于D435i相机的深度图，使用深度因子1000，经打印观测，所获取的深度值正常；
                        // std::cout << "[INFO]: The depth value is :  " << depth_val << std::endl;
                        ...
                        if (depth_val > PCL_MIN_DIST && depth_val < PCL_MAX_DIST)
                        {
                            ...
                            // 转换为ROS中的Pointcloud2格式，x，y，z的数据对应关系，
                            // 直接对应赋值图像显示不正常,在VINS-RGBD的RVIZ界面及PCL中显示又问退
                            // p.z = -b.y() * depth_val;
                            // p.x = depth_val;
                            // p.y = b.x() * depth_val;
                ...
                // posegraph.pubKeyframePose(pose_msg,header);
                posegraph.PointsToPointCloud(tmp,header);
                
                m_pointcloud2.unlock();
                ...
                // posegraph.PointsToPointCloud(cv_ptrRGB->image,depth,cx,cy,fx,fy,ROW,COL);
                // m_camera_use.unlock();
                //std::cout << "    point_depth size:    " << point_3d_depth.size() << std::endl;
                //debug: ROS_WARN("Depth points count: %d", count_);

                // 通过frame_index标记对应帧
                // add sparse depth img to this class
                // 相比于mono添加了point_3d_depth数据,主要是用来后面的八叉树地图的创建
                // 对于T、R是引用传参，能够修改T、R的值，故经过以下函数之后，其值是经过优化的;
                cv::Mat image_temp = _image.clone();
                KeyFrame* keyframe = new KeyFrame(pose_msg->header.stamp.toSec(), frame_index, T, R, image, image_temp,depth,point_3d_depth,
...
int main(int argc, char **argv)
{
    ...
    cv::FileNode Name = fsSettings["projection_parameters"];
    cx = static_cast<double>(Name["cx"]);
    cy = static_cast<double>(Name["cy"]);
    fx = static_cast<double>(Name["fx"]);
    fy = static_cast<double>(Name["fy"]);
        
    std::cout << "cx test:  " << cx << "cy test:  " << cy << std::endl;
