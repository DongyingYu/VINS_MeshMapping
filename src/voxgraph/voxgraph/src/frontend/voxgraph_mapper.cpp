...
void VoxgraphMapper::getParametersFromRos() {
  ...
  //param()函数参数的含义依次是：launch中的关键字；实际的变量名字；变量默认值
  //在launch文件中未获取到变量信息时，会使用默认值参数
  ...
  // 把submap_creation_interval的值读取到interval_temp中
  ...
  // 通过ROS params参数传入，怎么传？如何设置？
  XmlRpc::XmlRpcValue T_base_sensor_xml;
  // 把T_base_link_sensor值读取到T_base_sensor_xml
  // 应该是需要在yaml文件中添加并设置T_base_link_sensor值
  // getParam()函数可以从参数服务器获取参数值。如果成功，变量s和num的值将会被修改为参数值，
  // 函数返回true；如果不成功（譬如参数服务器没有设置这个参数），变量s和num将保持原值，函数会返回false
  ...
  if (!get_sensor_calibration_from_tfs) {
    ROS_INFO_STREAM("  Get sensor calibration from ROS params   ");
    Transformation T_B_C;
    kindr::minimal::xmlRpcToKindr(T_base_sensor_xml, &T_B_C);
    std::cout << "   initial value of T_B_C    " << T_B_C << std::endl;
...
void VoxgraphMapper::subscribeToTopics() {
  ...
  std::cout << "     subscribeToTopics function test     " <<std::endl;
  //ｍａｙｂｅ　需要输入里程计节点信息  设置里程计信息，在map_tracker.cpp函数中有用到
  //参数为空表示使用tf方式；  是否需要在launch文件中添加里程计信息，并写入launch文件？
  map_tracker_.subscribeToTopics(
      nh_, nh_private_.param<std::string>("odometry_input_topic","odometry_input_topic"));
}
