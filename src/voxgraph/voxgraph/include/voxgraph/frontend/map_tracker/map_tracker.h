...
class MapTracker {
 ...
 private:
  ...
  // 结合VINS-RGBD对此进行改动，改为false；跑官方给出的激光数据集或者使用TF信息运行改为true
  bool use_odom_from_tfs_ = false;
  ...
  // 结合VINS-RGBD对此进行改动，改为false；跑官方给出的激光数据集或者使用TF信息运行改为true
  bool use_sensor_calibration_from_tfs_ = false;
  // 在voxgraph_mapper.cpp中的判断语句，如果检测到有标定参数给出，该布尔型自动设置为false。
